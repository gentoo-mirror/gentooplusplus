...
read -p "[ QUESTION ] Would you like to install the Apache configuration? (type Y or y if yes) " -n 1 -r
echo    #
if [[ $REPLY =~ ^[Yy]$ ]]
then
    ...
    if [[ $SELFSIGNED -eq 1 ]]; then
        ...
        sed -i "s,/etc/letsencrypt/live/${funkwhale-sn}/privkey.pem,/etc/ssl/private/localhost.key," /etc/apache2/sites-available/funkwhale.conf
    fi
fi
